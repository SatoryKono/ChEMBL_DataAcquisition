...
Example
-------
>>> from mylib.chembl_client import get_target
>>> get_target("CHEMBL25")
...
def get_targets(ids: Iterable[str]) -> pd.DataFrame:
    """Fetch target records for ``ids`` in a single request.
    ...
    -------
    pandas.DataFrame
        DataFrame with one row per identifier.
    """
    ...
    try:
        ...
        logger.warning("Bulk target request failed: %s", exc)
        return pd.DataFrame(columns=TARGET_FIELDS)

    try:
        data = response.json()
    except ValueError as exc:  # pragma: no cover - malformed JSON
        logger.warning("Failed to decode JSON for targets: %s", exc)
        return pd.DataFrame(columns=TARGET_FIELDS)

    items = data.get("targets") or data.get("target") or []
    records = [_parse_target_record(item) for item in items]
